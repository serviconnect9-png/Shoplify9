...
async function loadDropshipStore(data) {
    ...
    if (!APP.userProfile) {
        container.innerHTML = '<p style="text-align:center;padding:40px;">Please login to view your store</p>';
        return;
    }
    ...
    container.innerHTML = '<p style="text-align:center;padding:40px;">Loading your store...</p>';
        ...
        container.innerHTML = `
            <div style="padding:15px;">
                <div style="text-align:center;padding:20px;background:linear-gradient(135deg,${storeColor},${storeColor}dd);border-radius:12px;color:${textColor};margin-bottom:15px;">
                    ...
                    <p style="opacity:0.8;">${APP.userProfile.storeBio || 'Dropship Store'}</p>
                </div>
...
async function loadPublicDropshipStore(username) {
    ...
    try {
        ...
        console.log('✅ Found dropshipper:', dropshipperId, dropshipper.storeName);
                                ...
                                return `
                                    <div class="product-card" onclick="viewStoreProductDetail('${p.id}','${p.name.replace(/'/g,"\\'")}','${p.price}','${p.minPrice}','${img}','${p.originalProductId}','${dropshipperId}')">
                                        ...
                                        <div class="product-card-info">
                                            ...
                                            <div style="font-size:10px;color:#666;">${p.stock > 0 ? p.stock + ' in stock' : 'In stock'}</div>
                                            <button class="btn-gold" style="width:100%;margin-top:6px;font-size:11px;padding:7px;" 
                                                    onclick="event.stopPropagation();addStoreProductToCart('${p.id}','${p.name.replace(/'/g,"\\'")}','${p.price}','${p.minPrice}','${img}','${p.originalProductId}','${dropshipperId}')">
                                                🛒 Add to Cart
                                            </button>
                                        </div>
                                    </div>`;
        ...
        console.log('✅ Public store loaded successfully');
...
function loadRecruitAffiliates() {
    ...
    Promise.all([
        db.collection('dropship_products').where('dropshipperId', '==', APP.userProfile.uid).where('status', '==', 'active').get(),
        ...
        container.innerHTML = `
            <div style="padding:15px;"><h3>📢 Recruit Affiliates</h3><p style="color:#666;margin-bottom:15px;">Select product and affiliates</p>
            <div class="input-group" style="margin-bottom:15px;"><label>Select Product</label><select id="recruit-product" class="input-field">${myProducts.map(p => `<option value="${p.originalProductId}">${p.name} - ${formatCurrency(p.price)}</option>`).join('')}</select></div>
            <div class="input-group" style="margin-bottom:15px;"><label>Commission (4-10%)</label><input type="number" id="recruit-commission" class="input-field" value="5" min="4" max="10"></div>
            <h4>Verified Affiliates (${verifiedAffiliates.length})</h4>
            <div id="affiliates-list" style="max-height:300px;overflow-y:auto;">${verifiedAffiliates.map(a => `<div style="display:flex;align-items:center;gap:10px;padding:10px;border-bottom:1px solid #f0f0f0;"><input type="checkbox" id="aff-${a.id}" value="${a.id}" style="width:18px;height:18px;"><img src="${a.photoURL||'/app-icon.png'}" style="width:40px;height:40px;border-radius:50%;"><div style="flex:1;"><div style="font-weight:600;">${a.displayName||a.username}</div><div style="font-size:11px;color:#666;">${a.totalSales||0} sales</div></div>${a.isAppVerified?'<span style="color:#20D5EC;">✓</span>':''}</div>`).join('')}</div>
            <button class="btn-gold btn-full" style="margin-top:15px;" onclick="sendRecruitRequests()">📤 Send Requests</button></div>`;
        
        window._recruitProductId = myProducts[0]?.originalProductId;
        window._recruitProduct = myProducts[0];
    });
}
...
async function sendRecruitRequests() {
    ...
    try {
        ...
        for (const aid of selectedAffiliates) {
            ...
            await createNotification(aid, '📢 Recruitment!',
                `${APP.userProfile.displayName || APP.userProfile.username} wants you to promote "${product?.name || 'a product'}" at ${commission}%`, '📢', 'notifications');
        }
...
console.log('✅ dropship.js fully loaded - ONESHOPLIFY Dropship System Ready');
